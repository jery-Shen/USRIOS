...
@implementation CRC
+(NSData *)getCRC:(NSData *)data{
    Byte buf[data.length];
    [data getBytes:&buf length:data.length];
    int crcValue = CRC16(buf,(int)data.length);
    buf[data.length]=crcValue>>8;   //CRC高位
    buf[data.length+1]=crcValue;    //CRC低位
    return [NSData dataWithBytes:buf length:sizeof(buf)+2];
}
@end
